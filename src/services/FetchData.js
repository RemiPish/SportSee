...
export default function FetchData(service, userID, isMock = false) {
    ...
    useEffect(() => {
        if (!isMock) {
            ...
            switch (service) {
                case 'infoUser':
                    data = USER_MAIN_DATA.find((user) => user.id === Number(userID));
                    break;
                case 'dailyActivity':
                    data = USER_ACTIVITY.find((user) => user.userId === Number(userID));
                    break;
                case 'averageSessions':
                    data = USER_AVERAGE_SESSIONS.find((user) => user.userId === Number(userID));
                    break;
                case 'performance':
                    data = USER_PERFORMANCE.find((user) => user.userId === Number(userID));
                    break;
                default:
                    break;
            }
